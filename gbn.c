  ...
  struct msg message;
{
	...
	/*Using current_buffer to traverse the buffer while maintaing the front and rear positions of the buffer*/
	current_buffer = (front_buffer + next_seqnum - base);

	/*Send packets that are within the window*/
	while ((current_buffer != rear_buffer) && (next_seqnum < base + WINSIZE)){
  ...
  struct pkt packet;
{
	...
	/*Move the front of the buffer*/
	if (packet.acknum >= base){
		front_buffer = (front_buffer + (packet.acknum - base) + 1);
	...
	/*Shift base to the next of the packet for which ack is received*/
	...
	/*Start timer if base is not same as the next sequence number*/
	else{
		stoptimer(0);
		starttimer(0, TIMEOUT);
	}
...
void A_timerinterrupt()
{
	...
	buffered_pkts = next_seqnum - base;
	front_buffer_temp = front_buffer;
	
	/*start timer again*/
	starttimer (0, TIMEOUT);
	
	/*Resend packets that have been buffered and are in the window*/
	for (i = 0; i < buffered_pkts; i++){
  ...
  struct pkt packet;
{
	...
	/*Check if the packet is one that receiver is expecting*/
	...
	/*Make and send ack packet*/
        ...
        sndpkt_B.acknum = sndpkt_B.seqnum;
...
void B_init()
{
	...
	/*Make initial ack packet to handle the case where first packet is corrupted*/
	sndpkt_B.seqnum = 0;
	sndpkt_B.acknum = 0;

	for (i = 0; i < 20; i++){
		sndpkt_B.payload[i] = 'a';
	}
	
	checksum = calc_checksum (sndpkt_B);
	sndpkt_B.checksum = checksum;
