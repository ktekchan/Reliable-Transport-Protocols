...
int flag_is_waiting; 	/*To check if sender is waiting for an ack for previous packet*/
int flag_ACK_sent;	/*To check if receiver has sent an ack*/
int expected_acknum;
int expected_seqnum;
struct pkt sndpkt_A;
struct pkt sndpkt_B;
  ...
  struct msg message;
{
	...
	/*Check if sender is already waiting for an ack for the previous packet*/
	if (flag_is_waiting == 1){
		printf ("Sender A: Waiting for ACK for previous packet. Discarding new data\n"); 
	return;
	}

	/*Toggle sequence number*/
	if (sndpkt_A.seqnum == 1){
		sndpkt_A.seqnum = 0;
	}
	else 
		sndpkt_A.seqnum = 1;

	/*Make and send packet with the payload*/
	...
	/*Change flag to 1 indicating that the sender is waiting for an ack*/
	flag_is_waiting = 1;
	expected_acknum = sndpkt_A.seqnum;
	A_transport++;
...
void A_input(packet)
  struct pkt packet;
{
	/*Check if the ack received is the one sender is waiting for*/
	...
	/*Change flag to 0 indicating the receipt of an appropriate ack*/
	flag_is_waiting = 0;
	return;
}
...
{
	/*Resend packet on account of Timeout event*/
...
void A_init()
{
	...
	sndpkt_A.seqnum = 1;	/*Set to 1 as the program toggles the sequence number before the first check point*/
	flag_is_waiting = 0;
	return;
}
  ...
  struct pkt packet;
{
	...
	if (packet.seqnum != expected_seqnum){
		...
		if (flag_ACK_sent == 1){
			printf ("Receiver B: Resending previous ACK.\n");
			tolayer3 (1, sndpkt_B);
		}
		return;
	}
	...
	flag_ACK_sent = 1;	/*Change the flag to 1 indicating that the ack has been sent*/

	/*Toggle the expected sequence number value*/
